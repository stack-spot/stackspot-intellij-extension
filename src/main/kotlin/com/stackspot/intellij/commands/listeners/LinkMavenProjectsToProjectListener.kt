...
class LinkMavenProjectsToProjectListener(private val project: Project) : CommandRunner.CommandEndedListener {
    ...
    override fun notifyEnded() {
        ...
        if (projectBasePath != null) {
            ...
            projectDir.walk().filter {
                it.name in listOf("pom.xml")
            }.forEach {
                ApplicationManager.getApplication().invokeLater {
                    val openProjectProvider = MavenOpenProjectProvider()
                    openProjectProvider.linkToExistingProject(it.absolutePath, project)
                }
            }
        }
    }
